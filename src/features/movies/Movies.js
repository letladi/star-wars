...
import { fetchCastMemberInfo, resetCastList } from "../cast/castSlice";
import "./Movie.css";
...
export function Movies() {
  ...
  return (
    <>
          ...
          onChange={(e) => {
            ...
            if (i >= 0) {
              ...
              movie.characters.forEach((url) =>
                dispatch(fetchCastMemberInfo(url))
              );
            } else {
      ...
      {selectedMovie ? <MovieCrawl /> : <img src={logo} alt="Star Wars Logo" />}
    </>
  );
}
...
function MovieCrawl() {
  ...
  return (
    <div className="marquee">
      <div>
        <span>{selectedMovie && selectedMovie.opening_crawl}</span>
      </div>
    </div>
  );
}
